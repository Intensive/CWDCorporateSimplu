 ...
 * Define Constants 
 */  
define('CWD_SHORTNAME', 'cwd_boot'); // used to prefix the individual setting field id see cwd_options_page_fields()  
define('CWD_PAGE_BASENAME', 'cwd_boot-settings'); // the settings page slug

/* 
 * Specify Hooks/Filters 
 */  
add_action( 'admin_menu', 'cwd_add_menu_bootstraptheme' );  

/* 
 * Group scripts (js & css) 
 */  
function cwd_settings_scripts(){  
    wp_enqueue_style('cwd_theme_settings_css', get_template_directory_uri() . '/inc/assets/css/cwd_theme_settings.css');  
    wp_enqueue_script( 'cwd_theme_settings_js', get_template_directory_uri() . '/inc/assets/js/cwd_theme_settings.js', array('jquery'));  
} 

/* 
 * The Admin menu page 
 */  
function cwd_add_menu_bootstraptheme(){  

    $settings_output        = cwd_get_settings();  
    // collect our contextual help text  
    $cwd_contextual_help = $settings_output['cwd_contextual_help'];  
      
    // Display Settings Page link under the "Appearance" Admin Menu  
    $cwd_settings_page = add_theme_page(__('Options'), __('Options','cwd_textdomain'), 'manage_options', CWD_PAGE_BASENAME, 'cwd_settings_page_fn');  

    // contextual help  
    if ($cwd_settings_page) {  
        add_contextual_help( $cwd_settings_page, $cwd_contextual_help );  
    }  
    // css & js  
    add_action( 'load-'. $cwd_settings_page, 'cwd_settings_scripts' );
}
 ...
 * @return array 
 */  
function cwd_get_settings() {  
      
    $output = array();  
      
    // put together the output array   
    $output['cwd_option_name']       = 'cwd_options'; // the option name as used in the get_option() call.  
    $output['cwd_page_title']        = __( 'Options','cwd_textdomain'); // the settings page title  
    $output['cwd_page_sections']     = cwd_options_page_sections(); // the setting section  
    $output['cwd_page_fields']       = cwd_options_page_fields();   // the setting fields  
    $output['cwd_contextual_help']   = cwd_options_page_contextual_help();  // the contextual help  
 ...
 */  
function cwd_create_settings_field( $args = array() ) {  
    ...
    $defaults = array(  
        ...
        'section' => 'main_section',                     // the section this setting belongs to — must match the array key of a section in cwd_options_page_sections()  
    ...
    // additional arguments for use in form field output in the function cwd_form_field_fn!  
    $field_args = array(  
        'type'      => $type,  
        'id'        => $id,  
        'desc'      => $desc,  
    ...
    add_settings_field( $id, $title, 'cwd_form_field_fn', __FILE__, $section, $field_args );  
 ...
 */  
function cwd_settings_page_fn() {  
// get the settings sections array  
    $settings_output = cwd_get_settings();  
?>  
    <div class="wrap">  
        <div class="icon32" id="icon-options-general"></div>  
        <h2><?php echo $settings_output['cwd_page_title']; ?></h2>  
          
        <form action="options.php" method="post">  
            <?php   
            // http://codex.wordpress.org/Function_Reference/settings_fields  
            settings_fields($settings_output['cwd_option_name']);   
              
            // http://codex.wordpress.org/Function_Reference/do_settings_sections  
            do_settings_sections(__FILE__);   
            ?>  
            <p class="submit">  
                <input name="Submit" type="submit" class="button-primary" value="<?php esc_attr_e('Save Changes','cwd_textdomain'); ?>" />  
            </p>  
 ...
 */  
function cwd_register_settings(){  
      
    // get the settings sections array  
    $settings_output    = cwd_get_settings();  
    $cwd_option_name = $settings_output['cwd_option_name'];  
      
    //setting  
    // register_setting( $option_group, $option_name, $sanitize_callback );  
    register_setting($cwd_option_name, $cwd_option_name, 'cwd_validate_options' );  
      
    //sections  
    // add_settings_section( $id, $title, $callback, $page );  
    if(!empty($settings_output['cwd_page_sections'])){  
        // call the "add_settings_section" for each!  
        foreach ( $settings_output['cwd_page_sections'] as $id => $title ) {  
            add_settings_section( $id, $title, 'cwd_section_fn', __FILE__);  
        }  
    }  
      
    //fields  
    if(!empty($settings_output['cwd_page_fields'])){  
        // call the "add_settings_field" for each!  
        foreach ($settings_output['cwd_page_fields'] as $option) {  
            cwd_create_settings_field($option);  
        }  
    }  
}  
 ...
 * @return echoes output 
 */  
function  cwd_section_fn($desc) {  
    echo "<p>" . __('','cwd_textdomain') . "</p>";  
}  
 ...
 * @return echoes output 
 */  
function cwd_form_field_fn($args = array()) {  
    ...
    $cwd_option_name = $settings_output['cwd_option_name'];  
    $options            = get_option($cwd_option_name);  
    ...
    switch ( $type ) {  
        case 'text':  
            ...
            echo "<input class='regular-text$field_class' type='text' id='$id' name='" . $cwd_option_name . "[$id]' value='$options[$id]' />";  
        ...
        case "multi-text":  
            foreach($choices as $item) {  
                ...
                $item[0] = esc_html__($item[0], 'cwd_textdomain');  
                if (!empty($options[$id])) {  
                ...
                echo "<span>$item[0]:</span> <input class='$field_class' type='text' id='$id|$item[1]' name='" . $cwd_option_name . "[$id|$item[1]]' value='$value' /><br/>";  
            }  
        ...
        case 'textarea':  
            ...
            echo "<textarea class='textarea$field_class' type='text' id='$id' name='" . $cwd_option_name . "[$id]' rows='5' cols='30'>$options[$id]</textarea>";  
            echo ($desc != '') ? "<br /><span class='description'>$desc</span>" : "";           
        break;  
          
        case 'select':  
            echo "<select id='$id' class='select$field_class' name='" . $cwd_option_name . "[$id]'>";  
                foreach($choices as $item) {  
                    $value  = esc_attr($item, 'cwd_textdomain');  
                    $item   = esc_html($item, 'cwd_textdomain');  
        ...
        case 'select2':  
            ...
            foreach($choices as $item) {  
                ...
                $item = explode("|",$item);  
                $item[0] = esc_html($item[0], 'cwd_textdomain');  
        ...
        case 'checkbox':  
            echo "<input class='checkbox$field_class' type='checkbox' id='$id' name='" . $cwd_option_name . "[$id]' value='1' " . checked( $options[$id], 1, false ) . " />";  
            echo ($desc != '') ? "<br /><span class='description'>$desc</span>" : "";  
        break;  
        ...
        case "multi-checkbox":  
            foreach($choices as $item) {  
                ...
                echo "<input class='checkbox$field_class' type='checkbox' id='$id|$item[1]' name='" . $cwd_option_name . "[$id|$item[1]]' value='1' $checked /> $item[0] <br/>";  
            }  
            echo ($desc != '') ? "<br /><span class='description'>$desc</span>" : "";  
        break;  
    }  
 ...
 */  
add_action( 'admin_menu', 'cwd_add_menu_bootstraptheme' );  
add_action( 'admin_init', 'cwd_register_settings' ); 
 ...
 * @return array 
 */  
function cwd_validate_options($input) {  
    ...
    $options = $settings_output['cwd_page_fields'];  
                ...
                switch ( $option['class'] ) {  
                    ...
                    case 'numeric':  
                        ...
                        if(is_numeric($input[$option['id']]) == FALSE) {  
                            add_settings_error(  
                                ...
                                CWD_SHORTNAME . '_txt_numeric_error', // error ID  
                                __('Expecting a Numeric value! Please fix.','cwd_textdomain'), // error message  
                    ...
                    case 'multinumeric':  
                        ...
                        if($input[$option['id']] !=''){  
                            ...
                            $valid_input[$option['id']] = (preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) == 1) ? $input[$option['id']] : __('Expecting comma separated numeric values','cwd_textdomain');  
                        }else{  
                        ...
                        if($input[$option['id']] !='' && preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) != 1) {  
                            add_settings_error(  
                                $option['id'], // setting title  
                                CWD_SHORTNAME . '_txt_multinumeric_error', // error ID  
                                __('Expecting comma separated numeric values! Please fix.','cwd_textdomain'), // error message  
                                'error' // type of message  
                            );  
                        }  
                    break;  
                    ...
                    case 'email':  
                        ...
                        $input[$option['id']]       = trim($input[$option['id']]); // trim whitespace  
                        if($input[$option['id']] != ''){  
                            $valid_input[$option['id']] = (is_email($input[$option['id']])!== FALSE) ? $input[$option['id']] : __('Invalid email! Please re-enter!','cwd_textdomain');  
                        }elseif($input[$option['id']] == ''){  
                            $valid_input[$option['id']] = __('This setting field cannot be empty! Please enter a valid email address.','cwd_textdomain');  
                        }  
                        ...
                        if(is_email($input[$option['id']])== FALSE || $input[$option['id']] == '') {  
                            add_settings_error(  
                                $option['id'], // setting title  
                                CWD_SHORTNAME . '_txt_email_error', // error ID  
                                __('Please enter a valid email address.','cwd_textdomain'), // error message  
                                'error' // type of message  
                            );  
                        }  
                    break;  
 ...
 */  
function cwd_show_msg($message, $msgclass = 'info') {  
 ...
 * 
 * @return calls cwd_show_msg() 
 */  
function cwd_admin_msgs() {  
      
    // check for our settings page - need this in conditional further down  
    $cwd_settings_pg = strpos($_GET['page'], CWD_PAGE_BASENAME);  
    ...
    if(current_user_can ('manage_options') && $cwd_settings_pg !== FALSE && !empty($set_errors)){  
  
        // have our settings succesfully been updated?   
        if($set_errors[0]['code'] == 'settings_updated' && isset($_GET['settings-updated'])){  
            cwd_show_msg("<p>" . $set_errors[0]['message'] . "</p>", 'updated');  
        ...
        // have errors been found?  
            ...
            foreach($set_errors as $set_error){  
                ...
                cwd_show_msg("<p class='setting-error-message' title='" . $set_error['setting'] . "'>" . $set_error['message'] . "</p>", 'error');  
            }  
        }  
    }  
}  
...
// admin messages hook!  
add_action('admin_notices', 'cwd_admin_msgs');  
 ...
 * Contextual Help 
 */  
function cwd_options_page_contextual_help() {  
      
    $text   = "<h3>" . __('CWD Settings - Contextual Help','cwd_textdomain') . "</h3>";  
    $text   .= "<p>" . __('Contextual help goes here. You may want to use different html elements to format your text as you want.','cwd_textdomain') . "</p>";  
